...
import { MediaPlaceholderElement } from '@/components/editor/ui/media-placeholder-element-app';
...
import { AIAnchorElement } from '@/registry/default/potion-ui/ai-anchor-element';
...
import { commentsPlugin } from './plugins/comments/comments-pugin';
  ...
  return usePlateEditor<Value, (typeof editorPlugins)[number]>(
    {
      ...
      plugins: [
        ...(readOnly
          ? // DIFF Potion don't need discussion plugin
            viewPlugins.filter((plugin) => plugin.key !== 'discussion')
          : [
              ...copilotPlugins,
              ...editorPlugins.filter((plugin) => plugin.key !== 'discussion'),
              blockSelectionScrollPlugin,
            ]),
        commentsPlugin,
        suggestionPlugin,
